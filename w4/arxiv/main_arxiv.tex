...
The integration tests verify the domain injection pipeline through two complementary paths. Early iterations programmatically constructed the agriculture profile by calling `DomainConfig::agriculture()` to ensure deterministic, CI-reproducible behavior without file-system dependencies. The final TOML-driven path loads the profile from the external `agri\_profile.toml` file via `Config::loader().without\_env().load\_str()`, adding approximately 1--12~ms to initialization. Both paths are exercised in the test suite, confirming that the domain logic can be injected either programmatically or declaratively. The retrieval benchmark evaluates the configured pipeline; the 210 queries cover all ten crops defined in the preset, and the knowledge base (`agricultural\_diseases.jsonl` plus `extended\_diseases.jsonl`) is accessed via the same `devkit\_query` tool that would be used in a production TOML-driven deployment.
...
Baseline-Random simulates an agent that has no access to structured agricultural context---a scenario analogous to deploying a general-purpose cloud LLM in a region with poor connectivity and no local knowledge base. The proposed system demonstrates that injecting a lightweight `DomainConfig` at runtime can elevate diagnostic retrieval to perfect top-1 accuracy on structured symptoms, while keeping latency below 20~ms. It is worth emphasizing that Baseline-CropOnly---which uses only the crop name---already achieves 100\% Hit@3 on this small, balanced knowledge base; the proposed method's added value is the precise top-1 ranking (100\% Hit@1) and near-perfect keyword recall (99.52\%) obtained through symptom-level scoring. This performance profile suggests that local-first, configuration-driven agents are technically viable for real-time field advisory use, even on modest edge hardware, provided the knowledge base is well-structured.
...
Multiple Authors.
\newblock {AGRARIAN}: A self-organizing cyber-physical system for sustainable
  agriculture.
\newblock {\em Discover Internet of Things}, 2025.
\newblock Springer Nature, doi:10.1007/s43926-025-00205-6.
