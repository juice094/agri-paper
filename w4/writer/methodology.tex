...
It is important to note the distinction between architectural capability and current validation practice. Clarity's `Config::loader()` infrastructure can indeed deserialize complete profiles---including `DomainConfig` objects---from external TOML files. The `agri\_profile.toml` artifact in the project repository demonstrates this target format. However, to ensure reproducibility in CI and eliminate file-path dependencies, the integration tests (`agriculture\_validation.rs`, `agriculture\_end\_to\_end.rs`) programmatically construct the agriculture profile by calling `DomainConfig::agriculture()`. The retrieval benchmark evaluates the resulting configured pipeline; the 210 queries cover all ten crops defined in the preset, and the knowledge base (`agricultural\_diseases.jsonl` plus `extended\_diseases.jsonl`) is accessed via the same `devkit\_query` tool that would be used in a TOML-driven deployment.
